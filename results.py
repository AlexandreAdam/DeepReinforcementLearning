...
path1 = './run/models/'
path2 = './results/figures/intra_architecture_CNN/'
EPISODES = 5
version_list = os.listdir(path1)
version_list.sort(key=lambda l: grp('(0-9+)', l))

keys = [i for i in range(len(version_list))]
values = [0]*len(version_list)
points_dict = dict(zip(keys, values))

j=0

# Each versions will play 5 games against each other versions.
for player_idx, _ in enumerate(version_list):
    m_tmp = player1_NN.read('run', player_idx + 1)
    player1_NN.model.set_weights(m_tmp.get_weights())
    player1 = Agent('player1', env.state_size, env.action_size, config.MCTS_SIMS, config.CPUCT, player1_NN)

    for opponent_idx, _ in enumerate(version_list):
        if player_idx <= opponent_idx:
            continue
        else:
            m_tmp = player2_NN.read('run', opponent_idx + 1)
